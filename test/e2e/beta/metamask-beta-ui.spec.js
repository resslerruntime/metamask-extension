...
describe("MetaMask", function() {
  ...
  describe("New UI setup", async function() {
    ...
    it("switches to first tab", async function() {
      ...
      try {
        networkSelector = await findElement(
          driver,
          By.css("#network_component")
        );
      } catch (e) {
    ...
    it("uses the local network", async function() {
      ...
      const networks = await findElements(
        driver,
        By.css(".dropdown-menu-item")
      );
    ...
    it("selects the new UI option", async () => {
      ...
      const button = await findElement(
        driver,
        By.xpath("//p[contains(text(), 'Try Beta Version')]")
      );
      await button.click();
      await delay(regularDelayMs);
      ...
      const continueBtn = await findElement(
        driver,
        By.css(".welcome-screen__button")
      );
  ...
  describe("Going through the first time flow", () => {
    it("accepts a secure password", async () => {
      const passwordBox = await findElement(
        driver,
        By.css(".create-password #create-password")
      );
      const passwordBoxConfirm = await findElement(
        driver,
        By.css(".create-password #confirm-password")
      );
      const button = await findElement(
        driver,
        By.css(".create-password button")
      );
    ...
    it("clicks through the ToS", async () => {
      // terms of use
      const canClickThrough = await driver
        .findElement(By.css(".tou button"))
        .isEnabled();
      assert.equal(canClickThrough, false, "disabled continue button");
      const bottomOfTos = await findElement(
        driver,
        By.linkText("Attributions")
      );
    ...
    it("reveals the seed phrase", async () => {
      const byRevealButton = By.css(
        ".backup-phrase__secret-blocker .backup-phrase__reveal-button"
      );
      ...
      seedPhrase = await driver
        .findElement(By.css(".backup-phrase__secret-words"))
        .getText();
    ...
    async function retypeSeedPhrase(words, wasReloaded) {
      try {
        ...
        const word0 = await findElement(
          driver,
          By.xpath(`//button[contains(text(), '${words[0]}')]`),
          10000
        );
        ...
        const word1 = await findElement(
          driver,
          By.xpath(`//button[contains(text(), '${words[1]}')]`),
          10000
        );
        ...
        const word2 = await findElement(
          driver,
          By.xpath(`//button[contains(text(), '${words[2]}')]`),
          10000
        );
        ...
        const word3 = await findElement(
          driver,
          By.xpath(`//button[contains(text(), '${words[3]}')]`),
          10000
        );
        ...
        const word4 = await findElement(
          driver,
          By.xpath(`//button[contains(text(), '${words[4]}')]`),
          10000
        );
        ...
        const word5 = await findElement(
          driver,
          By.xpath(`//button[contains(text(), '${words[5]}')]`),
          10000
        );
        ...
        const word6 = await findElement(
          driver,
          By.xpath(`//button[contains(text(), '${words[6]}')]`),
          10000
        );
        ...
        const word7 = await findElement(
          driver,
          By.xpath(`//button[contains(text(), '${words[7]}')]`),
          10000
        );
        ...
        const word8 = await findElement(
          driver,
          By.xpath(`//button[contains(text(), '${words[8]}')]`),
          10000
        );
        ...
        const word9 = await findElement(
          driver,
          By.xpath(`//button[contains(text(), '${words[9]}')]`),
          10000
        );
        ...
        const word10 = await findElement(
          driver,
          By.xpath(`//button[contains(text(), '${words[10]}')]`),
          10000
        );
        ...
        const word11 = await findElement(
          driver,
          By.xpath(`//button[contains(text(), '${words[11]}')]`),
          10000
        );
        await word11.click();
        await delay(tinyDelayMs);
    ...
    it("clicks through the deposit modal", async () => {
      ...
      const closeModal = await findElement(
        driver,
        By.css(".page-container__header-close")
      );
  ...
  describe("Send ETH from inside MetaMask", () => {
    it("starts to send a transaction", async function() {
      ...
      const inputAddress = await findElement(
        driver,
        By.css('input[placeholder="Recipient Address"]')
      );
      const inputAmount = await findElement(
        driver,
        By.css(".currency-display__input")
      );
      ...
      const configureGas = await findElement(
        driver,
        By.css(".send-v2__gas-fee-display button")
      );
  ...
  describe("Deploy contract and call contract methods", () => {
    ...
    it("creates a deploy contract transaction", async () => {
      ...
      const txListItem = await findElement(
        driver,
        By.xpath(`//span[contains(text(), 'Contract Deployment')]`)
      );
    ...
    it("renders the correct ETH balance", async () => {
      const balance = await findElement(
        driver,
        By.css(".tx-view .balance-display .token-amount")
      );
      await delay(regularDelayMs);
      if (process.env.SELENIUM_BROWSER !== "firefox") {
        await driver.wait(
          until.elementTextMatches(balance, /^92.*ETH.*$/),
          10000
        );
  ...
  describe("Add a custom token from a dapp", () => {
    ...
    it("picks the newly created Test token", async () => {
      const addCustomToken = await findElement(
        driver,
        By.xpath("//div[contains(text(), 'Custom Token')]")
      );
  ...
  describe("Send token from inside MetaMask", () => {
    ...
    it("opens customizes gas modal", async () => {
      await driver.wait(
        until.elementLocated(By.css(".send-v2__customize-gas__title"))
      );
      const save = await findElement(
        driver,
        By.xpath(`//button[contains(text(), 'Save')]`)
    ...
    it("finds the transaction in the transactions list", async function() {
      ...
      if (process.env.SELENIUM_BROWSER !== "firefox") {
        await driver.wait(
          until.elementTextMatches(txValues[0], /50\sTST/),
          10000
        );
      }

      const txStatuses = await findElements(driver, By.css(".tx-list-status"));
      const tx = await driver.wait(
        until.elementTextMatches(txStatuses[0], /Confirmed|Failed/),
        10000
      );
  ...
  describe("Send a custom token from dapp", () => {
    ...
    it("customizes gas", async () => {
      ...
      const gasFeeInputs = await findElements(
        driver,
        By.css(".confirm-detail-row__eth")
      );
    ...
    it("finds the transaction in the transactions list", async function() {
      ...
      const walletBalance = await findElement(
        driver,
        By.css(".wallet-balance")
      );
      ...
      // or possibly until we use latest version of firefox in the tests
      if (process.env.SELENIUM_BROWSER !== "firefox") {
        const tokenBalanceAmount = await findElement(
          driver,
          By.css(".token-balance__amount")
        );
  ...
  describe("Approves a custom token from dapp", () => {
    ...
    it("displays the token approval data", async () => {
      ...
      assert.equal(
        confirmDataText.match(
          /0x095ea7b30000000000000000000000002f318c334780961fb129d2a6c30d0763d9a5c97/
        )
      );
      ...
      const approvalWarning = await findElement(
        driver,
        By.css(".confirm-page-container-warning__warning")
      );
