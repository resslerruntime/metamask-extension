...
module.exports = class MetamaskController extends EventEmitter {
    ...
    const providerOpts = {
      static: {
        ...
        eth_sendTransaction: (payload, next, end) => {
          ...
          nodeify(
            this.txController.newUnapprovedTransaction,
            this.txController
          )(txParams, { origin }, end);
        }
      },
      // account mgmt
      getAccounts: cb => {
        const isUnlocked = this.keyringController.memStore.getState()
          .isUnlocked;
        const result = [];
  ...
  async checkHardwareStatus(deviceName) {
    switch (deviceName) {
      ...
      default:
        throw new Error(
          "MetamaskController:checkHardwareStatus - Unknown device"
        );
    }
  }
  ...
  async forgetDevice(deviceName) {
    switch (deviceName) {
      case "trezor":
        ...
        if (!keyring) {
          throw new Error(
            "MetamaskController:forgetDevice - Trezor Hardware keyring not found"
          );
        }
  ...
  signTypedMessage(msgParams) {
    ...
    // sets the status op the message to 'approved'
    // and removes the metamaskId for signing
    return this.typedMessageManager
      .approveMessage(msgParams)
      .then(cleanMsgParams => {
        // signs the message
  ...
  restoreOldLostAccounts(migratorOutput) {
    ...
    if (lostAccounts) {
      this.configManager.setLostAccounts(
        lostAccounts.map(acct => acct.address)
      );
  ...
  setupPublicConfig(outStream) {
    pump(asStream(this.publicConfigStore), outStream, err => {
      if (err) log.error(err);
    });
  }
