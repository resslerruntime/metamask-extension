...
BalanceComponent.prototype.renderBalance = function() {
  ...
  const formattedBalance = balanceValue
    ? formatBalance(balanceValue, 6, needsParse)
    : "...";
...
BalanceComponent.prototype.renderFiatValue = function(formattedBalance) {
  ...
  const fiatDisplayNumber = this.getFiatDisplayNumber(
    formattedBalance,
    conversionRate
  );
...
BalanceComponent.prototype.renderFiatAmount = function(
  fiatDisplayNumber,
  fiatSuffix,
  fiatPrefix
) {
  const shouldNotRenderFiat =
    fiatDisplayNumber === "N/A" || Number(fiatDisplayNumber) === 0;
  if (shouldNotRenderFiat) return null;

  const upperCaseFiatSuffix = fiatSuffix.toUpperCase();

  const display = currencies.find(
    currency => currency.code === upperCaseFiatSuffix
  )
