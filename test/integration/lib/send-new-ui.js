...
async function customizeGas(assert, price, limit, ethFee, usdFee) {
  const sendGasOpenCustomizeModalButton = await queryAsync(
    $,
  ...
  const customizeGasPriceInput = (await queryAsync(
    $,
  ...
  const customizeGasLimitInput = (await queryAsync(
    $,
    ".send-v2__gas-modal-card"
  ))
    .last()
  ...
  const customizeGasSaveButton = await queryAsync(
    $,
  ...
  assert.equal(
    (await findAsync(
      sendGasField,
      ".currency-display__input-wrapper > input"
    )).val(),
    ethFee,
  ...
  assert.equal(
    (await findAsync(sendGasField, ".currency-display__converted-value"))[0]
      .textContent,
    usdFee,
...
async function runSendFlowTest(assert, done) {
  ...
  const sendScreenButton = await queryAsync(
    $,
    "button.btn-primary.hero-balance-button"
  );
  ...
  const sendFromDropdownList = await queryAsync(
    $,
    ".send-v2__from-dropdown__list"
  );
  assert.equal(
    sendFromDropdownList.children().length,
    4,
    "send from dropdown shows all accounts"
  );
  sendFromDropdownList.children()[1].click();

  sendFromFieldItemAddress = await queryAsync(
    $,
    ".account-list-item__account-name"
  );
  assert.equal(
    sendFromFieldItemAddress[0].textContent,
    "Send Account 2",
    "send from field dropdown changes account name"
  );

  const sendToFieldInput = await queryAsync(
    $,
    ".send-v2__to-autocomplete__input"
  );
  sendToFieldInput[0].focus();

  const sendToDropdownList = await queryAsync(
    $,
    ".send-v2__from-dropdown__list"
  );
  assert.equal(
    sendToDropdownList.children().length,
    5,
    "send to dropdown shows all accounts and address book accounts"
  );
  ...
  assert.equal(
    ...
    "Insufficient funds.",
    "send should render an insufficient fund error message"
  );
  ...
  assert.equal(
    errorMessage.length,
    0,
    "send should stop rendering amount error message after amount is corrected"
  );
  ...
  const sendButton = await queryAsync(
    $,
    "button.btn-primary.btn--large.page-container__footer-button"
  );
  ...
  const confirmFromName = (await queryAsync(
    $,
    ".sender-to-recipient__sender-name"
  )).first();
  assert.equal(
    confirmFromName[0].textContent,
    "Send Account 4",
    "confirm screen should show correct from name"
  );

  const confirmToName = (await queryAsync(
    $,
    ".sender-to-recipient__recipient-name"
  )).last();
  assert.equal(
    confirmToName[0].textContent,
    "Send Account 3",
    "confirm screen should show correct to name"
  );

  const confirmScreenRowFiats = await queryAsync(
    $,
    ".confirm-detail-row__fiat"
  );
  const confirmScreenGas = confirmScreenRowFiats[0];
  assert.equal(
    confirmScreenGas.textContent,
    "$3.60",
    "confirm screen should show correct gas"
  );
  const confirmScreenTotal = confirmScreenRowFiats[1];
  assert.equal(
    confirmScreenTotal.textContent,
    "$2,405.36",
    "confirm screen should show correct total"
  );

  const confirmScreenBackButton = await queryAsync(
    $,
  ...
  const sendFromDropdownListInEdit = await queryAsync(
    $,
    ".send-v2__from-dropdown__list"
  );
  sendFromDropdownListInEdit.children()[2].click();

  const sendToFieldInputInEdit = await queryAsync(
    $,
    ".send-v2__to-autocomplete__input"
  );
  ...
  const sendAmountFieldInputInEdit = sendAmountFieldInEdit.find(
    ".currency-display__input"
  );
  sendAmountFieldInputInEdit.val("1.0");
  reactTriggerChange(sendAmountFieldInputInEdit[0]);

  const sendButtonInEdit = await queryAsync(
    $,
    ".btn-primary.btn--large.page-container__footer-button"
  );
  assert.equal(
    sendButtonInEdit[0].textContent,
    "Next",
    "next button in edit rendered"
  );

  selectState.val("send new ui");
  reactTriggerChange(selectState[0]);

  const cancelButtonInEdit = await queryAsync(
    $,
    ".btn-default.btn--large.page-container__footer-button"
  );
