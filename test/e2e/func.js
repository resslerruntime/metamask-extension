...
function buildChromeWebDriver(extPath) {
  const tmpProfile = fs.mkdtempSync(
    path.join(os.tmpdir(), "mm-chrome-profile")
  );
  return new webdriver.Builder()
    .withCapabilities({
      chromeOptions: {
        args: [`load-extension=${extPath}`, `user-data-dir=${tmpProfile}`],
...
async function getExtensionIdFirefox(driver) {
  ...
  const extensionId = await driver
    .findElement(
      By.css("dd.addon-target-info-content:nth-child(6) > span:nth-child(1)")
    )
    .getText();
  return extensionId;
}
