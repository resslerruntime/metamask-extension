...
const browserPlatforms = ["firefox", "chrome", "edge", "opera"];
...
const buildJsFiles = ["inpage", "contentscript", "background", "ui"];
...
function createTasksForBuildJsMascara({
  taskPrefix,
  devMode,
  bundleTaskOpts = {}
}) {
...
function createTasksForBuildJs({
  ...
  const subtasks = [];
  subtasks.push(
    gulp.parallel(buildPhase1.map(file => `${taskPrefix}:${file}`))
  );
...
buildJsFiles.forEach(jsFile => {
  gulp.task(
    `disc:${jsFile}`,
    discTask({ label: jsFile, filename: `${jsFile}.js` })
  );
});
...
gulp.task(
  "zip",
  gulp.parallel("zip:chrome", "zip:firefox", "zip:edge", "zip:opera")
);
...
gulp.task(
  ...
  gulp.series(
    "clean",
    "dev:scss",
    gulp.parallel("dev:mascara:js", "dev:copy", "dev:reload")
  )
);
...
gulp.task(
  ...
  gulp.series(
    "clean",
    "build:scss",
    gulpParallel("build:extension:js", "build:mascara:js", "copy")
  )
);
...
gulp.task(
  ...
  gulp.series(
    "clean",
    "build:scss",
    gulp.parallel("build:extension:js", "copy")
  )
);

gulp.task(
  "build:mascara",
  gulp.series("clean", "build:scss", gulp.parallel("build:mascara:js", "copy"))
);
