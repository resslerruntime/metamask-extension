...
async function getAllFileNames(dirName) {
  ...
  const subNameArrays = await promisify(async.map)(
    fullPathDirNames,
    getAllFileNames
  );
  let subNames = [];
  ...
  return [...fileNames.map(name => dirName + "/" + name), ...subNames];
}
        ...
        async (err, result) => {
          if (err) {
            ...
            const mapStateToPropsAssertionObject = returnObjectStrings[0]
              .replace(/\w+:\s\w+\([\w,\s]+\),/g, str => {
                ...
                return (
                  strKey +
                  ": 'mock" +
                  str.match(/^\w+/)[0].replace(/^./, c => c.toUpperCase()) +
                  ":mockState',\n"
                );
              })
              .replace(
            ...
            if (returnObjectStrings[1]) {
              mapDispatchToPropsMethodNames = returnObjectStrings[1]
                .match(/\s\w+:\s/g)
                .map(str => str.match(/\w+/)[0]);
            }
...
function generateDispatchMethodDescribeBlocks(methodArray) {
  return methodArray
    .map((methodName, index) =>
      generateDispatchMethodDescribeBlock(methodName, index)
    )
    .join("\n\n");
}
...
function generateContainerTest(
  sPath,
  {
    mapStateToPropsAssertionObject,
    mapDispatchToPropsMethodNames,
...
describe('${sPath.match(/^[^.]+/)} container', () => {
  ...
  describe('mapDispatchToProps()', () => {
    ...
    ${
      mapDispatchToPropsMethodNames
        ? generateDispatchMethodDescribeBlocks(mapDispatchToPropsMethodNames)
        : "delete"
    }
