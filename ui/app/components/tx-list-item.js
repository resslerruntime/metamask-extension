...
function mapDispatchToProps(dispatch) {
  return {
    setSelectedToken: tokenAddress =>
      dispatch(actions.setSelectedToken(tokenAddress)),
...
TxListItem.prototype.getAddressText = function() {
  const { address, txParams = {}, isMsg } = this.props;
  ...
  if (txDataName === "transfer" || address) {
    const addressToRender =
      txDataName === "transfer" ? checksummedValue : checksummedAddress;
    addressText = `${addressToRender.slice(0, 10)}...${addressToRender.slice(
      -4
    )}`;
  } else if (isMsg) {
...
TxListItem.prototype.getTokenInfo = async function() {
  ...
  ({ decimals, symbol } =
    tokens.filter(({ address }) => address === toAddress)[0] || {});
...
TxListItem.prototype.getSendTokenTotal = async function() {
  ...
  const showFiat =
    Boolean(totalInFiat) && currentCurrency.toUpperCase() !== symbol;
...
TxListItem.prototype.showRetryButton = function() {
  ...
  const currentNonceTxs = selectedAddressTxList.filter(
    tx => tx.txParams.nonce === currentNonce
  );
  const currentNonceSubmittedTxs = currentNonceTxs.filter(
    tx => tx.status === "submitted"
  );
  const currentSubmittedTxs = selectedAddressTxList.filter(
    tx => tx.status === "submitted"
  );
  const lastSubmittedTxWithCurrentNonce =
    currentNonceSubmittedTxs[currentNonceSubmittedTxs.length - 1];
  ...
  return (
    ...
    Date.now() - transactionSubmittedTime > 30000 &&
    currentTxIsLatest
  );
};
  ...
  return h(
    ...
    [
      h(`div.flex-column.tx-list-item-wrapper`, {}, [
        h(
          "div.tx-list-date-wrapper",
          {
            style: {}
          },
          [h("span.tx-list-date", {}, [dateString])]
        ),

        h(
          "div.flex-row.tx-list-content-wrapper",
          {
            style: {}
          },
          [
            h(
              "div.tx-list-identicon-wrapper",
              {
                style: {}
              },
              [
            ...
            h("div.tx-list-account-and-status-wrapper", {}, [
              h(
                "div.tx-list-account-wrapper",
                {
                  style: {}
                },
                [h("span.tx-list-account", {}, [this.getAddressText(address)])]
              ),

              h(
                "div.tx-list-status-wrapper",
                {
                  style: {}
                },
                [
            ...
            h(
              "div.flex-column.tx-list-details-wrapper",
              {
                style: {}
              },
              [
