...
async function captureAllScreens() {
  ...
  await driver
    .findElement(
      By.css(
        "#app-content > div > div.app-primary.from-right > div > div.flex-row.flex-center.flex-grow > p"
      )
    )
    .click();
  await delay(300);
  ...
  // enter seed phrase
  const seedPhraseButtons = await driver.findElements(
    By.css(".backup-phrase__confirm-seed-options > button")
  );
  const seedPhraseButtonWords = await Promise.all(
    seedPhraseButtons.map(button => button.getText())
  );
  for (const targetWord of seedPhraseWords) {
    const wordIndex = seedPhraseButtonWords.indexOf(targetWord);
    if (wordIndex === -1)
      throw new Error(
        `Captured seed phrase word "${targetWord}" not in found seed phrase button options ${seedPhraseButtonWords.join(
          " "
        )}`
      );
    await driver
      .findElement(
        By.css(
          `.backup-phrase__confirm-seed-options > button:nth-child(${wordIndex +
            1})`
        )
      )
      .click();
  ...
  await captureLanguageScreenShots(
    "confirm secret backup phrase - words selected correctly"
  );

  await driver
    .findElement(
      By.css(".backup-phrase__content-wrapper .first-time-flow__button")
    )
    .click();
  await delay(300);
  await captureLanguageScreenShots(
    "metamask post-initialize greeter screen deposit ether"
  );
  ...
  await driver
    .findElement(
      By.css(".wallet-view__name-container > .wallet-view__details-button")
    )
    .click();
  await delay(300);
  ...
  await driver
    .findElement(By.css(".account-modal__button:nth-of-type(2)"))
    .click();
  await delay(300);
  await captureLanguageScreenShots(
    "metamask account detail export private key screen - initial"
  );

  await driver
    .findElement(By.css(".private-key-password > input"))
    .sendKeys(password);
  await delay(300);
  await captureLanguageScreenShots(
    "metamask account detail export private key screen - password entered"
  );

  await driver
    .findElement(By.css(".btn-primary.btn--large.export-private-key__button"))
    .click();
  await delay(300);
  await captureLanguageScreenShots(
    "metamask account detail export private key screen - reveal key"
  );

  await driver.findElement(By.css(".export-private-key__button")).click();
  await delay(300);
  await captureLanguageScreenShots(
    "metamask account detail export private key screen - done"
  );
...
async function captureLanguageScreenShots(label) {
  const nonEnglishLocales = localesIndex.filter(
    localeMeta => localeMeta.code !== "en"
  );
  // take english shot
...
async function setLocale(code) {
  await driver.executeScript(
    "window.metamask.updateCurrentLocale(arguments[0])",
    code
  );
}
...
async function captureScreenShot(label) {
  ...
  await pify(fs.writeFile)(
    `${artifactDir}/${shotIndex} - ${label}.png`,
    screenshot,
    { encoding: "base64" }
  );
}
...
async function generateGif() {
  ...
  const stream = pngFileStream("./test-artifacts/screens/* (en).png")
    ...
    .pipe(
      fs.createWriteStream("./test-artifacts/screens/walkthrough (en).gif")
    );
