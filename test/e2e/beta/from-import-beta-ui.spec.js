...
describe("Using MetaMask with an existing account", function() {
  ...
  describe("New UI setup", async function() {
    ...
    it("use the local network", async function() {
      const networkSelector = await findElement(
        driver,
      ...
      const [localhost] = await findElements(
        driver,
        By.xpath(`//li[contains(text(), 'Localhost')]`)
      );
  ...
  describe("First time flow starting from an existing seed phrase", () => {
    it("imports a seed phrase", async () => {
      const [seedPhrase] = await findElements(
        driver,
        By.xpath(`//a[contains(text(), 'Import with seed phrase')]`)
      );
      ...
      const [seedTextArea] = await findElements(
        driver,
        By.css("textarea.import-account__secret-phrase")
      );
  ...
  describe("Imports an account with private key", () => {
    ...
    it("should show the imported label", async () => {
      const [importedLabel] = await findElements(
        driver,
        By.css(".wallet-view__keyring-label")
      );
  ...
  describe("Connects to a Hardware wallet", () => {
    ...
    it("should open the TREZOR Connect popup", async () => {
      const connectButtons = await findElements(
        driver,
        By.xpath(`//button[contains(text(), 'Connect to Trezor')]`)
      );
    ...
    it('should show the "Browser not supported" screen for non Chrome browsers', async () => {
      if (process.env.SELENIUM_BROWSER !== "chrome") {
        const title = await findElements(
          driver,
          By.xpath(`//h3[contains(text(), 'Your Browser is not supported...')]`)
        );
        assert.equal(title.length, 1);

        const downloadChromeButtons = await findElements(
          driver,
          By.xpath(`//button[contains(text(), 'Download Google Chrome')]`)
        );
