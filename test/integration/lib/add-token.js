...
async function runAddTokenFlowTest(assert, done) {
  ...
  let addTokenButton = await queryAsync(
    $,
    "button.btn-primary.wallet-view__add-token-button"
  );
  ...
  assert.equal(
    addTokenTitle[0].textContent,
    "Add Tokens",
    "add token title is correct"
  );

  // Cancel Add Token
  const cancelAddTokenButton = await queryAsync(
    $,
    "button.btn-default.btn--large.page-container__footer-button"
  );
  ...
  assert.ok(
    $(".wallet-view")[0],
    "cancelled and returned to account detail wallet view"
  );

  // Return to Add Token Screen
  addTokenButton = await queryAsync(
    $,
    "button.btn-primary.wallet-view__add-token-button"
  );
  assert.ok(addTokenButton[0], "add token button present");
  ...
  const tokenImageProp = tokenWrapper
    .find(".token-list__token-icon")
  ...
  assert.ok(
    $("button.btn-primary.btn--large")[0],
    "confirm add token button found"
  );
  ...
  assert.ok(
    tokenImageUrl.indexOf(heroBalance.find("img").attr("src")) > -1,
    "token added"
  );
  ...
  assert.equal(
    addTokenTabs[1].textContent,
    "Custom Token",
    "Custom Token tab present"
  );
  ...
  nativeInputValueSetter.call(
    customInput,
    "0x177af043D3A1Aed7cc5f2397C70248Fc6cDC056c"
  );
