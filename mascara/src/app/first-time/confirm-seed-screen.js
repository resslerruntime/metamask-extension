...
class ConfirmSeedScreen extends Component {
  ...
  render() {
    ...
    const isValid =
      seedWords === selectedSeeds.map(([_, seed]) => seed).join(" ");
                  ...
                  <div>
                    ...
                    <div className="backup-phrase__confirm-seed-options">
                      {shuffledSeeds.map((word, i) => {
                        const isSelected = selectedSeeds.filter(
                          ([index, seed]) => seed === word && index === i
                        ).length;
                          ...
                          <button
                            ...
                            onClick={() => {
                              if (!isSelected) {
                                ...
                                this.setState({
                                  selectedSeeds: selectedSeeds.filter(
                                    ([index, seed]) =>
                                      !(seed === word && index === i)
                                  )
                                });
