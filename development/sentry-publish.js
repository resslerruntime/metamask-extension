...
async function start() {
  ...
  if (versionAlreadyExists) {
    console.log(
      `Version "${VERSION}" already exists on Sentry, aborting sourcemap upload.`
    );
    return;
  }
  ...
  await exec(
    `for FILEPATH in ./dist/chrome/*.js; do [ -e $FILEPATH ] || continue; export FILE=\`basename $FILEPATH\` && echo uploading $FILE && sentry-cli releases --org 'metamask' --project 'metamask' files ${VERSION} upload $FILEPATH metamask/$FILE; done;`
  );
  console.log(`uploading sourcemaps Sentry release "${VERSION}"...`);
  await exec(
    `sentry-cli releases --org 'metamask' --project 'metamask' files ${VERSION} upload-sourcemaps ./dist/sourcemaps/ --url-prefix 'sourcemaps'`
  );
