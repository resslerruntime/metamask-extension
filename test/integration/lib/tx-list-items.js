...
async function runTxListItemsTest(assert, done) {
  ...
  assert.equal(
    $(unapprovedTx).hasClass("tx-list-pending-item-container"),
    true,
    "unapprovedTx has the correct class"
  );

  const retryTx = txListItems[1];
  const retryTxLink = await findAsync(
    $(retryTx),
    ".tx-list-item-retry-container span"
  );
  assert.equal(
    retryTxLink[0].textContent,
    "Taking too long? Increase the gas price on your transaction",
    "retryTx has expected link"
  );
  ...
  const failedTxRenderedStatus = await findAsync(
    $(failedTx),
    ".tx-list-status"
  );
  assert.equal(
    failedTxRenderedStatus[0].textContent,
    "Failed",
    "failedTx has correct label"
  );

  const shapeShiftTx = txListItems[5];
  const shapeShiftTxStatus = await findAsync(
    $(shapeShiftTx),
    ".flex-column div:eq(1)"
  );
  assert.equal(
    shapeShiftTxStatus[0].textContent,
    "No deposits received",
    "shapeShiftTx has correct status"
  );

  const confirmedTokenTx = txListItems[6];
  const confirmedTokenTxAddress = await findAsync(
    $(confirmedTokenTx),
    ".tx-list-account"
  );
  assert.equal(
    confirmedTokenTxAddress[0].textContent,
    "0xE7884118...81a9",
    "confirmedTokenTx has correct address"
  );

  const rejectedTx = txListItems[7];
  const rejectedTxRenderedStatus = await findAsync(
    $(rejectedTx),
    ".tx-list-status"
  );
  assert.equal(
    rejectedTxRenderedStatus[0].textContent,
    "Rejected",
    "rejectedTx has correct label"
  );
}
